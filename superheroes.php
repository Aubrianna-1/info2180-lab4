...
<?php
$info=trim(filter_var($_GET['query'],FILTER_SANITIZE_STRING));
        // starting out with no input
        $inList = false;
        foreach ($superheroes as $hero){
            if(($hero["alias"]===$info) || ($hero["name"]===$info)){
                $inList = true;
            ...
            } elseif($info===""){ //does not work with == 
                echo"<ul><li>{$hero['alias']}</li> </ul>";
            }
        }
        // if there is no input and the hero is not in the list
        if($info !== "" && $inList === false){ //does not work with == 
